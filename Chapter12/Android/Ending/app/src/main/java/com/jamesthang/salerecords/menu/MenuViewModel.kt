...
class MenuViewModel @Inject constructor(
    ...
    fun onOrderClick(context: Context) {
        ...
        db.collection("users").document(userId).collection("sales")
            ...
            .addOnSuccessListener {
                ...
                // Log order creation success event
                val firebaseAnalytics = FirebaseAnalytics.getInstance(context)
                val numberOfItems = orderRecords.size
                val totalSaleAmount = orderRecords.sumOf { it.total() }
                val bundle = Bundle().apply {
                    putInt("number_of_items", numberOfItems)
                    putDouble("total_value", totalSaleAmount)
                }
                firebaseAnalytics.logEvent("create_order_success", bundle)
            ...
            .addOnFailureListener {exception ->
                ...
                // Log order creation failure event
                val firebaseAnalytics = FirebaseAnalytics.getInstance(context)
                val bundle = Bundle().apply {
                    putString("error_message", exception.message)
                }
                firebaseAnalytics.logEvent("create_order_failure", bundle)

                // Handle notification here
                // For example, show a toast or log the error
                Log.e("OrderError", "Error creating order: ${exception.message}")
            }
